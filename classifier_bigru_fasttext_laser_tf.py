"""
Classifier using BiGRU w/ pretrained FastText embeddings & LASER embeddings
"""
import time
import pandas as pd
import numpy as np
...
def build_classifier_model(embedding_matrix):
    input_1 = layers.Input(shape=(MAX_SEQ_LEN,), dtype=np.int32)
    input_2 = layers.Input(shape=(1024,), dtype=np.float32)

    embedding_layer = layers.Embedding(VOCAB_SIZE + 1,
                                       EMBEDDING_DIMS,
                                       weights=[embedding_matrix],
                                       trainable=False)
    embedded_input = embedding_layer(input_1)
    ...
    dense_2 = layers.Dense(128, activation='relu')(input_2)
    combined_output = layers.concatenate([gru_output, dense_2])
    prob = layers.Dense(NUM_OUTPUTS, activation='sigmoid')(combined_output)

    bigru_model = Model([input_1, input_2], prob)
    bigru_model.summary()
...
def train_driver(train_tuple,
                 ...
                 test_tuple,
                 embedding_matrix):
    train_features, train_laser_features, train_labels = train_tuple
    val_features, val_laser_features, val_labels = val_tuple
    test_features, test_laser_features, test_ids = test_tuple
    ...
    for curr_epoch in range(NUM_EPOCHS):
        classifier.fit([train_features, train_laser_features], train_labels,
                       batch_size=BATCH_SIZE,
                       epochs=1,
                       verbose=1)

        if len(val_labels):
            val_preds = classifier.predict([val_features, val_laser_features])
            val_roc_auc_score = roc_auc_score(val_labels, val_preds)
            print(val_roc_auc_score)

        test_preds = classifier.predict([test_features, test_laser_features]).squeeze()
...
if __name__ == '__main__':
    ...
    train_driver([train_features, train_laser_features, train_labels],
                 [val_features, val_laser_features, val_labels],
                 [test_features, test_laser_features, test_ids],
