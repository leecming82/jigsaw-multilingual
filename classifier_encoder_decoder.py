...
class ClassifierHead(torch.nn.Module):
    ...
    def forward(self, x, y):
        decoder_output = self.base_model(input_ids=x,
                                         decoder_input_ids=y)
        logits = self.fc(decoder_output[1][:, 0, :])
...
def predict_evaluate(model, data_tuple, epoch, score=False):
    ...
    with torch.no_grad():
        for batch_idx_start in range(0, len(data_tuple[-1]), BATCH_SIZE):
            ...
            batch_encoder_features = torch.tensor(data_tuple[0][0][batch_idx_start:batch_idx_end]).cuda()
            batch_decoder_features = torch.tensor(data_tuple[0][1][batch_idx_start:batch_idx_end]).cuda()
            batch_preds = model(batch_encoder_features, batch_decoder_features)
